...
- [x] T001 Inicializar proyecto Astro 7 + TypeScript: `package.json`, `astro.config.mjs` con
      `output: "server"` y adaptador `@astrojs/vercel` (Node serverless, no Edge — research.md
      #1), `tsconfig.json` en modo estricto
- [x] T002 [P] Instalar dependencias de runtime y dev: `@supabase/supabase-js`, `zod`,
      `resend`, `qrcode`, `sharp`, `@playwright/test` (dev), en `package.json`
- [x] T003 [P] Crear `supabase/config.toml` para desarrollo local y `.env.example` con
      `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `OPS_SECRET` (research.md #9)
- [x] T004 [P] Configurar `playwright.config.ts` apuntando a `npm run build && npm run
      preview` (no al dev server), per quickstart.md
- [x] T005 [P] Crear helper de throttling CDP en `tests/e2e/utils/network-throttle.ts`
...
- [x] T006 [P] Crear script de seed `scripts/seed-regalo-demo.ts`: inserta un regalo
...
- [x] T007 Crear estructura de carpetas vacías per plan.md → Project Structure:
...
- [x] T008 Escribir migración `supabase/migrations/0001_regalos.sql`: tablas `slugs`
      ...
      data-model.md
- [x] T009 [P] Implementar cliente Supabase server-only (service role, nunca en bundle de
      cliente) en `src/lib/supabase.ts`
- [x] T010 [P] Implementar generación de slug en `src/lib/slugs.ts`: alfabeto
...
- [x] T011 [P] Implementar esquemas `zod` y tipos TS del contenido versionado v1 en
...
- [x] T012 [P] Implementar `src/lib/contenido/embeds.ts`: lista blanca de dominios
...
- [x] T013 [P] Crear `src/styles/base.css`: reset, `max-width: 100vw` / `overflow-x: hidden`
      a nivel layout (FR-020), baseline `prefers-reduced-motion`, foco visible global
- [x] T014 [P] Crear `src/temas/tokens-nocturno.css` — custom properties del tema íntimo y
      oscuro, pares fondo/texto verificados AA (research.md #13)
- [x] T015 [P] Crear `src/temas/tokens-papel.css` — custom properties del tema claro y
      analógico, pares fondo/texto verificados AA (research.md #13)
- [x] T016 [P] Crear `src/temas/tokens-luminoso.css` — custom properties del tema alegre y
      con color, pares fondo/texto verificados AA (research.md #13)
- [x] T017 Implementar `src/lib/regalos.ts`: `obtenerRegaloPublicado(slug)` (valida
...
- [x] T018 [P] [US1] `tests/e2e/carta-larga.spec.ts` — carta de ~4.000 caracteres se muestra
      completa, sin scroll horizontal (Edge Case)
- [x] T019 [P] [US1] `tests/e2e/contenido-extremo.spec.ts` — nombre con emoji/acentos
      ...
      #20)
- [x] T020 [P] [US1] `tests/e2e/fecha-futura.spec.ts` — `fechaInicio` futura: contador cuenta
      regresiva y nunca negativo en el cruce (Edge Case, research.md #18)
- [x] T021 [P] [US1] `tests/e2e/bloque-desconocido.spec.ts` — tipo no reconocido se omite en
...
- [x] T022 [P] [US1] `tests/e2e/render-sin-imagenes.spec.ts` — con imágenes bloqueadas, 100%
      del texto sigue legible (SC-005)
- [x] T023 [P] [US1] `tests/e2e/sin-scroll-horizontal.spec.ts` — anchos 320–1440 px, cero
      scroll horizontal en ningún bloque (SC-002, FR-020)
- [x] T024 [P] [US1] `tests/e2e/lcp.spec.ts` — dos mediciones bajo 3G simulada + gama baja
...
- [x] T025 [P] [US1] Crear `src/bloques/Portada.astro` — solo contenido de portada:
...
- [x] T026 [P] [US1] Crear `src/bloques/Contador.astro` — `fechaInicio`; `<script>` vanilla
...
- [x] T027 [P] [US1] Crear `src/bloques/Momento.astro` — `titulo?`, `texto` (requerido, se
      omite si está vacío), `fecha?`, `foto?` con `alt` requerido y el mismo encuadre
      `object-fit: contain` que `Galeria.astro` (research.md #20)
- [x] T028 [P] [US1] Crear `src/bloques/Galeria.astro` — `fotos: Foto[]`, cada `<img>` con
...
- [x] T029 [P] [US1] Crear `src/bloques/Cancion.astro` — usa `resolverEmbed(url)` de
...
- [x] T030 [P] [US1] Crear `src/bloques/Carta.astro` — `texto` (requerido); layout que se
      adapta en alto a textos largos (~4.000 caracteres) sin romper el diseño; se omite si el
      texto está vacío
- [x] T031 [P] [US1] Crear `src/bloques/Pregunta.astro` — `texto`, `respuesta?`; `<script>`
      vanilla que alterna `aria-expanded` para revelar la respuesta al tocar
- [x] T032 [P] [US1] Crear `src/bloques/Cierre.astro` — `texto?`
- [x] T033 [US1] Crear `src/bloques/registro.ts` — `Record<string, Componente>` mapeando los 8
      `tipo` a sus componentes; cualquier `tipo` ausente del mapa se resuelve a `undefined`
      (FR-006, research.md #15) — depende de T025–T032
- [x] T034 [US1] Crear `src/layouts/RegaloLayout.astro` — recibe `tema` + `receta`, aplica el
...
- [x] T035 [P] [US1] Crear `src/pages/no-disponible.astro` — página de "no disponible" sin
      detalles técnicos, para código inexistente, no publicado o vencido (FR-002)
- [x] T036 [P] [US1] Crear `src/pages/api/abrir/[slug].ts` — `POST`: resuelve `slug` a un
...
- [x] T037 [US1] Crear `src/pages/r/[slug].astro` — ruta pública inmutable: llama
...
- [x] T038 [P] [US2] `tests/e2e/revision-y-aprobacion.spec.ts` — `/revisar/[id]` renderiza
...
- [x] T039 [US2] Crear `src/pages/revisar/[id].astro` — llama
      ...
      T017
- [x] T040 [US2] Crear `src/pages/api/regalos/[id]/aprobar.ts` — `POST`: si `estado !==
      ...
      depende de T009
- [x] T041 [US2] En `src/pages/revisar/[id].astro`, agregar el estado post-aprobación: cuando
...
- [x] T042 [P] [US3] `tests/e2e/primera-apertura.spec.ts` — primera apertura registra
...
- [x] T043 [US3] En `src/pages/revisar/[id].astro`, mostrar `primera_apertura_en` cuando no es
...
- [x] T044 [P] Crear fixture dorado `tests/fixtures/contenido-v1.json` — documento `{ v: 1,
...
- [x] T045 [P] `tests/e2e/compatibilidad-versiones.spec.ts` — renderiza
...
- [x] T046 [P] `tests/e2e/contraste-aa.spec.ts` — calcula el ratio de contraste WCAG de cada
...
- [x] T047 Crear `src/pages/api/regalos/[id]/eliminar.ts` — `DELETE` protegido por
...
- [x] T048 Ejecutar la validación completa de `quickstart.md`: los 3 escenarios manuales (US1,
...
**Purpose**: brecha encontrada al comparar el código actual contra spec.md tras completarse
la Fase 7. No reemplaza ni reordena ninguna tarea previa.

- [x] T055 Agregar un test Playwright que verifique que un slug inexistente y un regalo
      publicado pero vencido (`vencimiento` en el pasado) devuelven ambos la página de "no
      disponible" en `/r/[slug]` — hoy solo el disparador "en_revision" de FR-002 tiene
      cobertura (`revision-y-aprobacion.spec.ts`); `obtenerRegaloPublicado` ya implementa el
      filtro de `vencimiento`, pero ningún test lo ejerce — per FR-002, FR-017, US1/AC4
      (missing)
